...
BASE_IRI = 'https://github.com/PR0CK0/ProvTracer/tree/main/procko.provtracer/grapher/provtracer-o.ttl#'
...
class ProvGraphBuilder:
  def __init__(self, outputPath):
    ...
    self.PTO = Namespace(BASE_IRI)
    self.BFO = Namespace(BFO_IRI)
    self.graph.bind('prov', PROV)
    self.graph.bind('xsd', XSD)
    self.graph.bind('pto', self.PTO)
    ...
    self.graph.add((self.PTO.WorkSession, RDF.type, URIRef('http://www.w3.org/2002/07/owl#Class')))
    self.graph.add((self.PTO.WorkSession, RDF.type, PROV.Activity))
    self.graph.add((self.PTO.WorkTimeslice, RDF.type, URIRef('http://www.w3.org/2002/07/owl#Class')))
    self.graph.add((self.PTO.WorkTimeslice, RDF.type, PROV.Activity))
    self.graph.add((self.PTO.TimeslicePacket, RDF.type, URIRef('http://www.w3.org/2002/07/owl#Class')))
    self.graph.add((self.PTO.TimeslicePacket, RDF.type, PROV.Entity))

    self.knowledgeWorker = self.PTO.KnowledgeWorker
  ...
  def initializeSession(self, outputPath):
    ...
    self.workSession = URIRef(f'{self.PTO}WorkSession_{self.sessionTimestamp}')

    self.graph.add((self.workSession, RDF.type, self.PTO.WorkSession))
    self.graph.add((self.workSession, self.PTO.sessionName, Literal(self.sessionName)))
  ...
  def createWorkTimeslice(self, timestamp):
    if timestamp not in self.workTimeslices:
      workTimeslice = self.PTO[f'WorkTimeslice_{timestamp}']
      self.graph.add((workTimeslice, RDF.type, self.PTO.WorkTimeslice))
  ...
  def parseContexts(self):
    ...
    if os.path.exists(primerPath):
      with open(primerPath, 'r', encoding = 'utf-8', errors = 'replace') as file:
        ...
        self.graph.add((self.workSession, self.PTO.primerContext, Literal(primerContent)))
      ...
      if os.path.isdir(eventPath):
        for eventFile in os.listdir(eventPath):
          if eventFile.endswith('.txt'):
            filePath = os.path.join(eventPath, eventFile)
            timestamp = self.extractAndValidateTimestamp(eventFile)
            if not timestamp:
              continue

            if eventType == 'keyboard_events':
              from recorder.userEventStringers.keyboardEventsStringer import KeyboardEventsStringer
              try:
                stringer = KeyboardEventsStringer(filePath)
                eventContent = self.contextTokenLengthLimiter.limitLength(stringer.getKeyboardEventsString(), eventType)
              except Exception as e:
                print(f"Warning: Failed to parse keyboard event file {eventFile}: {e}")
                continue
            else:
              with open(filePath, 'r', encoding='utf-8', errors='replace') as file:
                rawContent = file.read()
                eventContent = self.contextTokenLengthLimiter.limitLength(self.sanitizeText(rawContent), eventType)
  ...
  def createTimeslicePacket(self, workTimeslice, timestamp):
    packet = self.PTO[f'TimeslicePacket_{timestamp}']
    self.graph.add((packet, RDF.type, self.PTO.TimeslicePacket))
  ...
  def addAttributeToTimeslicePacket(self, packet, attribute, value):
    self.graph.add((packet, self.PTO[attribute], Literal(value)))
